 ...
 * to plan a trip according to a map and the timetable of boat
 * transfers.
 *
 * From Riva
 ...
 * prevent infinite loops by limiting the allowed length of
...
trip(PT1, PT2) :- tt(Route), append(_, [PT1, PT2|_], Route).
...
% Again, here is our ll/2 predicate.
ll([], 0).
ll([_|Ls], N) :- N #> 0, N #= N0 + 1, ll(Ls, N0).
...
% schedule(Place1 at Time1, Place2 at Time2, Schedule, SchLen).
